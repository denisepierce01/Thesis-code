...
for Li = 1
    ...
    % CS_binedges = CS(1):5:CS(2);
    CS_binedges = -30:10:30;
    ...
    % LEG_G = legend([LEG_GROYNES],{'Existing Groyne','New Groynes','Channel','High Water Refuge'},'Location','southeast');
    LEG_G = legend([LEG_GROYNES],{'Existing Groyne','New Groynes','Channel Wall'},'Location','southeast');
    cb_bathy = colorbar('Location','SouthOutside');
    cb_bathy.Position = [0.780 0.41   0.10    0.02];
    tcb = title(cb_bathy,'z_b [m+NAP]');
    cb_bathy.TickDirection = 'out';

    title([NAMES.(LOCS{Li}) ' HW = 3 m'],'FontSize',14)
    xlim([XLIM.(LOCS{Li})])
    ylim([YLIM.(LOCS{Li})])
...
% cb.Limits = CS_binedges([1 end]);
% cb.AxisLocation = 'in';
% title(cb,{'% getijden',['onder ' num2str(LD_grens*100) ' cm/s']})
% FAX.Position = [-1 -1 0.1 0.1];
% 
