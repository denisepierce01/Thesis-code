...
KML = KML2Coordinates('p:\11210344-001-bathosszimm\04_Data\Ingrepen\9_8_2024\Nieuw.kml');
for ki = 1:length(KML)
    [Nieuw_x{ki},Nieuw_y{ki}] = convertCoordinates(KML{ki}(:,1),KML{ki}(:,2),'CS2.code',28992,'CS1.code',4326);
end
KML = KML2Coordinates('p:\11210344-001-bathosszimm\04_Data\Ingrepen\9_8_2024\Aangepast.kml');
for ki = 1:length(KML)
    [Aangepast_x{ki},Aangepast_y{ki}] = convertCoordinates(KML{ki}(:,1),KML{ki}(:,2),'CS2.code',28992,'CS1.code',4326);
end
KML = KML2Coordinates('p:\11210344-001-bathosszimm\04_Data\Ingrepen\9_8_2024\Bestaand.kml');
for ki = 1:length(KML)
...
YLIM.BATH = [378  380.5];
XLIM.OSSE = [56 60];
YLIM.OSSE = [379.5 383.5];
XLIM.ZIMM = [64 67];
YLIM.ZIMM = [379 381];


% Zodra duidelijk is wat het beinvloedde gebied is, dan de randen interessegebied aanpassen voor verdere stats/analyses
...
KML = KML2Coordinates('p:\11210344-001-bathosszimm\04_Data\Ingrepen\afzonderlijk\Ossenisse_interessegebied.kml');
...
%             SA{Li} = subaxis(1,2,Li,'MB',0.32,'MT',0.04,'ML',-0.058,'MR',0.03,'PL',-0.02);
            SA{Li} = subaxis(1,2,Li,'MB',0.11,'MT',0.035,'ML',0.07,'MR',-0.025,'PL',0);
            hold on
            AS = gca;             
...
%             BV = ((ZVAK.Z.(['Y' num2str(Y_B)])-ZVAK.Z.(['Y' num2str(Y_A)])).*100)./(Y_B-Y_A).*I; % Bodemverandering in cm per jaar
            BV = ((ZVAK.Z.(['Y' num2str(Y_B)])-ZVAK.Z.(['Y' num2str(Y_A)])).*100./I); % Bodemverandering totaal in cm

            P=pcolor(ZVAK.X./1000,ZVAK.Y./1000,BV);
            P.LineStyle = 'none';
...
%             ylim([YLIM.(LOCS{Li})])
            ...
            if Li == 2
                ...
                CB.Position = [0.6597 0.2111 0.3000 0.0200];
...
%             LEG_G.Position(2) = 0.003;
        ...
        % Export [cm/yr] per period
        BV_OSSE = BV;
        BV_OSSE(ZVAK.X./1000<61) = NaN;
        BV_BATH = BV;
        BV_BATH(ZVAK.X./1000>69) = NaN;
        BV_ZIMM = BV;
        BV_ZIMM(ZVAK.X./1000>69) = NaN;
...
function format_ratios_and_positions(SA,LOCS,LEG_G,XLIM,T)
    ...
    SA{2}.Position(1) = A1p(1)+A1p(3)+0.07;
    SA{2}.Position(2) = A1p(2)+A1p(4)-A2p(4)+SA{2}.Position(2)-A2p(2);
    LEG_G.Position(1) = 1-LEG_G.Position(3)-0.002-0.0082;
    if strcmp(T,'T1')
        LEG_G.Position(2) = 0.003;
    else
        LEG_G.Position(2) = .07;
