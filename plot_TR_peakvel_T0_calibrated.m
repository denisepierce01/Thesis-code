...
png_dir = 'P:\11207654-internship-pierce-2026\03_Model\ModelOutput\T0\'; % Location of png's
...
for ss = 1:length(stations_vel)
    ...
    for ii = 1:length(folders)
        ...
        scatter(Model.TR, Model.peakVel, 80, royalBlue, 'filled', 'MarkerFaceAlpha', 0.8, 'DisplayName', folders{ii});     
...
%% Plot tiled format for Bath & Zimm
fig = figure();
% fig.Position = [100, 100, 1200, 900]; 

% 2. Create the layout and assign it to a variable 'tl'
tl = tiledlayout(5, 4, 'TileSpacing', 'loose', 'Padding', 'compact');
fig.Units = 'centimeters';
fig.Position(3:4) = [18 25]; % width x height in cm

% 3. Apply labels directly to your layout variable 'tl'
title(tl, 'Bath T0 Calibration', 'FontSize', 24, 'FontWeight', 'bold');
...
for ss = 1:18
    ...
    for ii = 1:length(folders)
        ...
        % Plot model points
        hold on;
        scatter(Model.TR, Model.peakVel, 20, royalBlue, 'filled', 'MarkerFaceAlpha', 0.8, 'DisplayName', folders{ii});
    end
...
% axes & labels
title(tl, 'Zimmerman T0 Calibration', 'FontSize', 24, 'FontWeight', 'bold');
xlabel(tl, 'Tidal Range [m]', 'FontSize', 18, 'FontWeight', 'bold');
ylabel(tl, 'Peak Velocity (Depth-Averaged) [m/s]', 'FontSize', 18, 'FontWeight', 'bold');
