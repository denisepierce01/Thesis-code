...
dirs.figures = 'p:\11207654-internship-pierce-2026\02_Data\Velocity_data_ADCP\Figures\Zimm_flow';
...
for k = 1:maxK
    ...
    % Setup Figure
    figName = sprintf('Direction_Hist_BATH_%d', k);
...
% outDir = fullfile('P:','11207654-internship-pierce-2026','02_Data','Velocity_data_ADCP','Figures','BATH_T0T1_histogram');
% if ~exist(outDir,'dir')
%     mkdir(outDir);
% end
% 
...
%     sgtitle(sprintf('BATH %s', nameStr), 'FontSize', 16, 'FontWeight', 'bold');
%     drawnow;
%     end
...
try
    ...
    outDir = fullfile('P:','11207654-internship-pierce-2026','02_Data','Velocity_data_ADCP','Figures','BATH_direction');
...
% 
% % =====================================================================
% % 3. GENERATE GEOGRAPHIC MAP PLOT OVER AERIAL TILES
% % =====================================================================
% name = 'pdok_luchtfoto';
% url = 'https://service.pdok.nl/hwh/luchtfotorgb/wmts/v1_0/Actueel_ortho25/EPSG:3857/${z}/${x}/${y}.jpeg';
% attribution = 'Background: © PDOK / Kadaster';
...
% geoplot(gx, station_Lat, station_Lon, 'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 7, 'DisplayName', 'Stations');
% 
% % Spatial scaling for geoaxes vector representation 
% spatial_scale_factor = 0.005; 
% scaled_U1 = plot_U1 .* arrow_length1 * spatial_scale_factor;
% scaled_V1 = plot_V1 .* arrow_length1 * spatial_scale_factor;
% scaled_U2 = plot_U2 .* arrow_length2 * spatial_scale_factor;
% scaled_V2 = plot_V2 .* arrow_length2 * spatial_scale_factor;
...
% if exist('geoquiver', 'file') == 2
%     q1 = geoquiver(gx, station_Lat, station_Lon, scaled_V1, scaled_U1, 0, ...
%                   'LineWidth', 2.5, 'Color', 'b', 'DisplayName', 'Primary Flow');
%     q2 = geoquiver(gx, station_Lat, station_Lon, scaled_V2, scaled_U2, 0, ...
%                   'LineWidth', 1.8, 'Color', 'r', 'LineStyle', '-', 'DisplayName', 'Secondary Flow');
% else
%     % Fallback manual plotting loop
%     endLat1 = station_Lat + scaled_V1;
%     endLon1 = station_Lon + scaled_U1;
%     for i = 1:numel(station_Lat)
%         if i == 1
%             geoplot(gx, [station_Lat(i), endLat1(i)], [station_Lon(i), endLon1(i)], '-', ...
%                     'Color', 'b', 'LineWidth', 2.5, 'DisplayName', 'Primary Flow');
%         else
%             geoplot(gx, [station_Lat(i), endLat1(i)], [station_Lon(i), endLon1(i)], '-', ...
%                     'Color', 'b', 'LineWidth', 2.5, 'HandleVisibility', 'off');
%         end
...
%         if i == 1
%             geoplot(gx, [station_Lat(i), endLat2(i)], [station_Lon(i), endLon2(i)], '-', ...
%                     'Color', 'r', 'LineWidth', 1.8, 'DisplayName', 'Secondary Flow');
%         else
%             geoplot(gx, [station_Lat(i), endLat2(i)], [station_Lon(i), endLon2(i)], '-', ...
%                     'Color', 'r', 'LineWidth', 1.8, 'HandleVisibility', 'off');
%         end
...
% 
% % =====================================================================
% % 4. FORCE RD TICK MARKS & LABELS OVER GEOAXES
% % =====================================================================
% % Determine tidy grid step lines in RD New kilometers (e.g., every 200m or 500m)
% rd_x_ticks = 70600:400:71800;
% rd_y_ticks = 378400:400:380600;
% 
% % Convert these explicit grid boundaries into their relative Lat/Lon positions
% % We evaluate the cross-sections using center anchors to establish perpendicular tick placement
% [~, lon_converted] = projinv(crs_rd, rd_x_ticks, repmat(mean(ylims_shifted), size(rd_x_ticks)));
% [lat_converted, ~] = projinv(crs_rd, repmat(mean(xlims), size(rd_y_ticks)), rd_y_ticks);
% 
% % Bind the newly calculated locations to the geographic frame properties
% gx.LongitudeTick = lon_converted;
% gx.LatitudeTick = lat_converted;
% 
% % Rewrite the degree labels into clean "km" strings (dividing by 1000)
% gx.LongitudeTickLabel = arrayfun(@(v) sprintf('%.1f', v/1000), rd_x_ticks, 'UniformOutput', false);
% gx.LatitudeTickLabel = arrayfun(@(v) sprintf('%.1f', v/1000), rd_y_ticks, 'UniformOutput', false);
% 
% % Formatting Text Layout Properties
% gx.FontSize = 14;
% title(gx, 'Flow Directions: T1', 'FontWeight', 'bold', 'FontSize', 22);
% xlabel(gx, 'RDx [km]', 'FontWeight', 'bold', 'FontSize', 18);
% ylabel(gx, 'RDy [km]', 'FontWeight', 'bold', 'FontSize', 18);
% 
% lgd = legend(gx, 'Location', 'southeast', 'FontSize', 15);
% set(lgd, 'Color', 'none', 'EdgeColor', 'none');
% 
% hold(gx, 'off');
...
ax.XTick = 69000:1000:73000;
ax.YTick = 378000:1000:381000;
...
% Text elements
title(ax, 'Flow Directions: Bath T_1', 'FontWeight', 'bold', 'FontSize', 30, 'Color', [0.1 0.1 0.1]);
...
%     mkdir(PNG_dir);
% end
% file_name_PNG = fullfile(PNG_dir, 'BathT1_ebb-flood_dir.png');
% 
