...
dirs.figures = 'P:\11207654-internship-pierce-2026\04_Scripts\HWvsU\Figures\';
...
for Li = 1
    ...
    for Ti = 1:length(Ts)       
        ...
        for i = 1:length(I)
            ...
            plot(XL,LD_grens.*[1 1],'--k'); %grens voor laag-hoog dynamisch in de Westerschelde
            ...
            if Ti == 2
              ...
              LEG = legend([plotpoints{1} plotpoints{2} fitlines{1} fitlines{2}],'T0 getij','T1 getij','Fit T0 getijden','Fit T1 getijden','AutoUpdate','off');
...
fprintf(['De absolute verschillen tussen T0 en T1 zijn relatief klein ' ...
         ...
         round(min_range), round(max_range));
...
fprintf(['Op de MPxx02 meetpunten is de maximale dieptegemiddelde snelheid af/toegenomen (gemiddeld met %.2f cm/s voor een hoogwaterstand van 2 en 3 m) \n'], ...
         round(avg_xx02));
...
fprintf(['Op de MPxx01 meetpunten is de maximale dieptegemiddelde snelheid af/toegenomen (gemiddeld met %.2f cm/s voor een hoogwaterstand van 2 en 3 m) \n'], ...
         round(avg_01xx));
...
for Li = 1
    ...
    LEG_G = legend([LEG_GROYNES],{'Existing Groynes','New Groynes','Channel Wall Defense','High Water Refuge'},'Location','southeast');
...
for Li = 1
   ...
   %remove excess-colors:
    ...
    LEG_G = legend([LEG_GROYNES],{'Bestaande strekdam','Nieuwe strekdam','Opgehoogde dam/geulwandverdediging','Hoogwatervluchtplaats'},'Location','southeast');
    cb_bathy = colorbar('Location','SouthOutside');
    cb_bathy.Position = [0.735 0.41    0.15    0.02];
    tcb = title(cb_bathy,'z_b [m+NAP]');
    cb_bathy.TickDirection = 'out'; 
...
cb.Limits = CS_binedges([1 end]);
cb.AxisLocation = 'in';
title(cb,{'% tides',['under ' num2str(LD_grens*100) ' cm/s']})
FAX.Position = [-1 -1 0.1 0.1];
...
fprintf(['In T0 lag %.2f procent van de snelheidspieken onder de 60 m/s, in T1 lag %.2f procent daaronder. \n'], ...
    round(avgT0_frac), round(avgT1_frac));

%% Histogram
% Bar chart comparing T0_frac and T1_frac
% Prepare data: only keep measurement indices that have both T0 and T1
hasBoth = ~isnan(T0_frac) & ~isnan(T1_frac);
T0_plot = T0_frac(hasBoth);
T1_plot = T1_frac(hasBoth);
Tdiff_plot = T1_plot - T0_plot;

% % Determine corresponding site names I for the kept indices.
% % Assume variable I (measurement/site names) exists in workspace from earlier code.
% % If not, try to get them from ADCP data for the current location LOCS{Li}
% if exist('I','var')
%     siteNamesAll = I;
% elseif exist('ADCP','var') && exist('LOCS','var') && exist('Li','var')
%     A = ADCP.(LOCS{Li});
%     siteNamesAll = fieldnames(A);
% else
%     siteNamesAll = arrayfun(@(n) sprintf('Site%d',n), 1:length(T0_frac), 'UniformOutput', false);
% end
% siteNames = siteNamesAll(hasBoth);

% figure;
% hb = bar([T0_plot(:) T1_plot(:)], 'grouped');
% hb(1).FaceColor = [0.6 0.8 1.0]; % pale blue for T0
% hb(2).FaceColor = [0 0.4470 0.7410]; % normal MATLAB blue for T1
% hold on
% % Add numeric labels above bars
% ngroups = size([T0_plot(:) T1_plot(:)],1);
% nbars = 2;
% groupwidth = min(0.8, nbars/(nbars+1));
% for i = 1:nbars
%     x = (1:ngroups) - groupwidth/2 + (2*i-1) * groupwidth/(2*nbars);
%     y = hb(i).YData;
%     % text(x, y + max([T0_plot;T1_plot])*0.02, arrayfun(@(v) sprintf('%.1f',v), y, 'UniformOutput',false), ...
%     %     'HorizontalAlignment','center','FontSize',8);
% end
% 
% % Label x-axis ticks with site names
% set(gca, 'XTick', 1:ngroups, 'XTickLabel', siteNames, 'XTickLabelRotation', 45);
% hold off
% xlabel('Measurement index');
% ylabel('Percent of tides under critical HW (%)');
% ylim([0, 105])
% legend({'T0','T1'}, 'Location','northwest', 'FontSize', 14);
% title(Bath )
% grid on
% box on

%% Find peak velocity at each site for T0 and T1
